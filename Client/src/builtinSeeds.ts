...
export const BUILTIN_SEEDS: WidgetDef[] = [
  ...
  {
    key: 'vfd-motor-panel', type: 'vfd-motor-panel', name: '变频电机控制面板',
    defaultWidth: 260, defaultHeight: 350,
    icon: getLucideIcon('sliders') || getLucideIcon('refresh-cw'), iconKind: 'lucide', iconColor: 'text-amber-500',
    description: '工业级变频电机复合控制卡片：集成启/停/复位、准备/运行/故障/信号指示、设定/反馈频率双轨监控与运行电流/负荷率警示。',
    category: 'equipment', renderKind: 'builtin', propSchema: [], isSystem: true, sortOrder: 65,
    defaultProps: () => ({
      ...baseProps('vfd-motor-panel'),
      motorName: '1# 变频主循环泵',
      motorTag: 'M101',
      panelStyle: 'slate-dark',
      panelAccentColor: '#38bdf8',
      ratedCurrent: 25.0,
      minFreq: 0.0,
      maxFreq: 50.0,
      freqStep: 1.0,
      ...
      resetVar: 'cmd_reset',
      readyVar: 'st_ready',
      runningVar: 'st_running',
      faultVar: 'st_fault',
      signalVar: 'comm_ok',
      spFreqVar: 'sp_freq',
      pvFreqVar: 'pv_freq',
      currentVar: 'pv_current',
      opDeviceId: null,
      showCurrent: true,
      showFrequency: true,
      showControls: true,
  ...
  },
  {
    ...
    defaultProps: () => ({
      ...
      dashboardItemBorderColor: '',
      dashboardItemBgColor: '',
