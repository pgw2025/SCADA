...
<template>
  <div v-if="open && device" class="fixed inset-0 z-50 overflow-hidden text-left">
    ...
    <div class="fixed inset-y-0 right-0 max-w-full flex pl-10">
      <div class="w-screen max-w-md bg-white dark:bg-slate-900 shadow-2xl border-l border-slate-200 dark:border-slate-800 flex flex-col select-none">
        <!-- Header -->
        <div class="px-5 py-4 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800">
          ...
          <button
            type="button"
            @click="emit('close')"
            class="p-1 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 cursor-pointer transition-colors"
          >
            <X class="w-4 h-4" />
          </button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-5 text-xs">
          ...
          <div class="p-3.5 rounded-xl border flex items-center justify-between"
            :class="device.status === 1 || device.status === 'online'
              ? 'bg-emerald-50/70 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800/60'
              : 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-700'">
            <div class="flex items-center gap-2.5">
              <div class="relative flex items-center justify-center">
                <span v-if="device.status === 1 || device.status === 'online'" class="w-3 h-3 rounded-full bg-emerald-500 animate-ping absolute opacity-75"></span>
                <span class="w-3 h-3 rounded-full" :class="device.status === 1 || device.status === 'online' ? 'bg-emerald-500' : 'bg-slate-400'"></span>
              </div>
              <div>
                <div class="font-bold text-sm" :class="device.status === 1 || device.status === 'online' ? 'text-emerald-700 dark:text-emerald-300' : 'text-slate-600 dark:text-slate-300'">
            ...
            <button
              type="button"
              @click="emit('toggleEnabled', device)"
              class="px-2.5 py-1 rounded-lg font-bold text-xs cursor-pointer transition-colors"
              :class="device.isEnabled
          ...
          <div class="space-y-2">
            ...
            <div class="bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 rounded-xl p-3 space-y-2 text-[11px]">
            ...
            <div class="bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 rounded-xl p-3 space-y-2 text-[11px]">
              <template v-if="device.connection">
                <div class="flex justify-between">
                  ...
                  <span class="font-medium text-violet-600 dark:text-violet-400">#{{ device.connection.id }} - {{ device.connection.controllerName || device.connection.controllerCode }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-slate-400">通信协议</span>
                  <span class="font-medium text-slate-800 dark:text-slate-200">{{ device.connection.protocolName || device.connection.protocolKey }}</span>
                </div>
                <div class="flex justify-between">
                ...
                <div class="flex justify-between">
                  ...
                  <span class="font-mono text-slate-600 dark:text-slate-400">{{ device.connection.timeoutMs }}ms / {{ device.connection.reconnectIntervalMs }}ms</span>
                </div>
              </template>
              <template v-else>
                ...
                <div v-if="device.ipAddress" class="flex justify-between">
                  ...
                  <span class="font-mono font-bold text-slate-800 dark:text-slate-200">{{ device.ipAddress }}:{{ device.port || 102 }}</span>
                </div>
          ...
          <div class="space-y-2">
            ...
            <div class="bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 rounded-xl p-3 space-y-2.5">
              <!-- Primary Model -->
              <div class="flex items-start gap-2 p-2 rounded-lg bg-sky-50/80 dark:bg-sky-950/40 border border-sky-100 dark:border-sky-900">
              ...
              <div v-if="device.models && device.models.filter(m => !m.isPrimary).length > 0" class="space-y-1.5 pt-1">
                ...
                <div
                  v-for="b in device.models.filter(m => !m.isPrimary)"
                  :key="b.id"
        ...
        <div class="p-4 bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3">
          <button
            type="button"
            @click="emit('variables', device)"
