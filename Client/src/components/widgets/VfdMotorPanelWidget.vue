...
// 已抽取为共享 composable useVfdPanelTheme，供变频电机控制面板 / 实时多变量看板等复用。
const panelStyle = computed(() => propOr('panelStyle', 'slate-dark'));
const panelAccentColor = computed(() => propOr('panelAccentColor', '#38bdf8'));
const { panelTheme: vfdPanelTheme, themeVars: vfdThemeVars } = useVfdPanelTheme({ panelStyle, panelAccentColor });
