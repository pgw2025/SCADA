  ...
  <div class="h-full overflow-y-auto p-4 sm:p-6 bg-slate-50/50 dark:bg-transparent text-[#1e293b] dark:text-slate-100 select-none">
    ...
    <div class="mt-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2.5 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs">
      ...
      <div class="ml-auto flex items-center gap-2">
        <button @click="loadList" class="text-[10px] text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 font-bold px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700 hover:border-slate-300 transition-all cursor-pointer inline-flex items-center gap-1">
    ...
    <div class="mt-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm text-left transition-colors">
      ...
      <table class="w-full text-xs">
        <thead>
          <tr class="bg-slate-50 dark:bg-slate-950/60 ring-1 ring-slate-100 dark:ring-slate-800 uppercase text-[10px] text-slate-400 dark:text-slate-500 font-bold tracking-wider">
            ...
            <th class="px-4 py-3 text-left">所属控制器</th>
            <th class="px-4 py-3 text-left">协议</th>
            <th class="px-4 py-3 text-left">地址</th>
            <th class="px-4 py-3 text-left">重连(ms)</th>
        ...
        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
          <tr v-for="c in paged" :key="c.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-all">
            <td class="px-4 py-3 font-bold text-slate-500 dark:text-slate-400">{{ c.id }}</td>
            <td class="px-4 py-3 font-sans font-bold text-slate-800 dark:text-white">{{ c.name }}</td>
            <td class="px-4 py-3 text-slate-500 dark:text-slate-400">
              <span class="font-mono text-slate-400 dark:text-slate-500">{{ c.controllerCode || '#' + c.controllerId }}</span>
              <span v-if="c.controllerName" class="ml-1">{{ c.controllerName }}</span>
            </td>
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3 font-mono text-slate-500 dark:text-slate-400">{{ endpointLabel(c) }}</td>
            <td class="px-4 py-3 font-mono text-slate-500 dark:text-slate-400">{{ c.reconnectIntervalMs }}</td>
            <td class="px-4 py-3">
              <span
              ...
              <span
                v-if="c.inUseByDevice"
                class="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border ml-1 bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-800"
              >
                <Link2 class="w-3 h-3" />
                已关联设备
              </span>
      ...
      <div v-if="filtered.length === 0 && !loading" class="py-12 text-center text-slate-400 dark:text-slate-500 text-xs">
        <Link2 class="w-8 h-8 mx-auto mb-2 opacity-30" />
        <span>暂无连接，点击右上角"添加连接"创建第一台设备连接</span>
      </div>
