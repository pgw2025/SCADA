...
| 阶段 | 名称 | 覆盖需求 | 依赖 |
|---|---|---|---|
| P0 | 准备与基线 | 分支、审计、类型放宽 | 无 |
| P1 | 后端模板管理 | 模板 CRUD / 种子 | P0 |
| P2 | 前端模板源切换 | 组件库动态化（不写死） | P1 |
| P3 | 管理页 + 导入导出 | 添加 / 删除 / 编辑 / 导入导出 | P2 |
| P4 | SVG 渲染引擎 | 零代码新建组件 | P3 |
| P5 | 属性面板 Schema 化 | InspectorPanel 收敛（可选） | P3 |
| P6 | 回归验收 | 全量回归 | P1～P5 |

> P1～P3 完成 即满足「不写死 + 增删改 + 导入导出」全部原始诉求；P4/P5 为增强能力，可按需排期。
