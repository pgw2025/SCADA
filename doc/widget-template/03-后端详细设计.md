...
> **实施状态：P1 已落地（迁移 + 24 种子 + CRUD + 导入导出 + SvgSanitizer）。下方标注「⚠ 实施偏差」处为实况回填。**
...
> ⚠ **实施偏差（导入防伪与透视修正，commit 170091e）**：实际 `ImportAsync` 对**所有**导入路径一律强制 `dto.IsSystem = false`（含 overwrite 命中存量时也覆盖为 false），即**不信任文件中的 isSystem 值**，杜绝导入文件伪造系统内置标记（审查 A11/R11 从严落地）。设计稿仅「overwrite 保留现有 IsSystem」已被此规则取代。
>
> ⚠ **实施偏差（import-bundle，commit e83f7f0 后续修正）**：`ImportBundle` 循环内逐条 `ImportAsync(new WidgetTemplateImportDto { Template = t, ConflictMode = "rename", Format = bundle.Format })` —— 实际**透传 `bundle.Format`**；设计稿代码漏传 Format 会导致 400（C2 缺陷，已修）。
    }
...
> ⚠ **实施偏差（C1 修正，commit e83f7f0 后续）**：实际 Create 不再回显入参 `dto`（未清洗的 SVG 会原样泄漏），而是入库后 `GetByIdAsync(id)` 读回持久化实体再 `CreatedAtAction(…, created)` 返回，保证响应体与库中清洗后的数据一致。
