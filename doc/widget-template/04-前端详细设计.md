...
> **实施状态：已全部落地（P2~P5）。本文档下方各节标注「⚠ 实施偏差」处为与设计稿不一致的实况回填。**
...
| `src/lucideIcons.ts` | **新增（实施）** | ⚠ 偏差：lucide 图标字典独立成叶子模块（环终结者），见 §1 |
| `src/propSchemas.ts` | **新增（P5）** | ⚠ 新增：内置组件属性 Schema（BUILTIN_SCHEMAS） |
| `components/inspector/PropSchemaForm.vue` | **新增（P5）** | ⚠ 新增：schema 驱动通用表单渲染器 |
...
> ⚠ **实施偏差（§1）**：`LUCIDE_ICON_MAP` / `getLucideIcon` 实际独立到 `src/lucideIcons.ts`（叶子模块，仅依赖 lucide-vue-next，作为环终结者），`builtinRenderers.ts` 仅 `export { getLucideIcon } from './lucideIcons'` 转发。设计意图不变：图标字典仍属编译期静态资产。
...
> ⚠ **实施偏差（§3，P5 修复）**：
> 1. `getWidgetDef` 的 store 命中不再裸返回 `WidgetTemplateDto`，而是经 `toDef()` 映射为 `WidgetDef` 再返回。原因：DTO 无 `defaultProps()` / `propSchema` / `icon` 等运行字段，裸返回会导致 InspectorPanel `typeDefaults` computed 调用 `def.defaultProps()` 抛 TypeError（P5 接入 schema 面板时暴露并修复）。
> 2. `toDef` 的 `propSchema` 字段实际经 `resolvePropSchema(t)` 解析：DB `propSchemaJson` 非空用 DB（自定义模板自建 schema），空则按 `templateKey → renderType` 回退 `BUILTIN_SCHEMAS`（内置 24 种子 DB 侧统一存 `[]`，schema 定义在前端 `propSchemas.ts`，见 §12）。
...
> ⚠ **实施偏差（§11）**：`bindSvgTemplate` 内类型断言实际为 `(ctx as unknown as Record<string, unknown>)`（消除 TS 强转告警），行为不变。

## 12. P5 实施追加：属性面板 Schema 化

> 本节为实施时新增（设计稿 §5.2 的落地形态），不在原设计稿编号内。

### 12.1 `src/propSchemas.ts`

```ts
export interface PropSchemaOption { value: string | number | boolean; label: string; }
...
- 覆盖全部 24 内置种子 + legacy `button`（键 = `templateKey`，复合键变体如 `title-header-tech-desktop` 独立成条）；
- 简单组件全量 schema 化；trend-chart / multi-var-dashboard / rounded-btn / nav-menu / var-display 五类复杂组件 schema 仅覆盖通用外观项，专属面板保留（与 §5.1 设计一致）；
- `InspectorPanel` 被替换的硬编码分支 = 8 处（6 处 `.includes()` 分组属性区 + sys-time + image 专属区），满足 P5 验收「≥ 8」量化目标。

### 12.2 `components/inspector/PropSchemaForm.vue`

- 五类控件（text/number/color/select/switch）渲染，`updateProp` 走 InspectorPanel 既有 emit 链路；
- 展示值三级回显：`props.props[key]` → `defaults[key]`（模板默认）→ `schema.default`；
- select 提交按选项原始类型（数值选项保持 number）；nullable 阈值输入留空提交 `null`。

### 12.3 InspectorPanel 接入

- `schemaItems` computed：`getWidgetDef(t)?.propSchema ?? BUILTIN_SCHEMAS[t] ?? []`（DB 模板 schema → 内置兜底）；
- 通用外观属性区由 `<PropSchemaForm :schema :props :defaults @update-prop>` 渲染；布局 / 绑定 / 事件三段式骨架与复杂组件专属面板（var-display 可设定 / trend-chart 多序列 / rounded-btn 按钮模式 / nav-menu 导航项 / multi-var-dashboard 看板项）保持不变。

### 12.4 管理页（P4 补全，对应 §10 渲染轨）

- RenderKind='svg' 时：SVG 源码 textarea + **实时预览**（注入示例值 42.5℃/开启/55%/运行色，走与画布相同的 sanitize+bind 链路）+ 「可用占位符」速查按钮（点击插入光标处）+ 256KB 长度校验（与后端 SvgSanitizer 限长一致，超限拒绝保存）；新建 svg 模板时 DefaultProps 预填绑定上下文默认值（审查 B5）。
