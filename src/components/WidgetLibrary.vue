...
<template>
  <div class="h-full flex flex-col bg-white border-r border-[#d9d9d9] text-[#262626]">
    <!-- Search Header -->
    <div class="p-4 border-b border-[#f0f0f0] bg-[#fafafa]">
      <h3 class="text-xs font-bold text-[#141414] uppercase tracking-wider mb-2.5 flex items-center gap-2">
        <Package class="w-4 h-4 text-[#1890ff]" />
        工业器件图库
      </h3>
      <div class="relative">
        <input
          ...
          placeholder="搜索工业器件..."
          v-model="searchTerm"
          class="w-full bg-white border border-[#d9d9d9] rounded py-1.5 pl-8 pr-3 text-xs text-slate-800 placeholder-gray-400 focus:outline-none focus:border-[#1890ff] focus:ring-1 focus:ring-[#1890ff]"
        />
    ...
    <!-- Tabs -->
    <div class="flex text-center border-b border-[#f0f0f0] px-1 text-[11px] bg-[#fafafa]">
      <button
        @click="activeTab = 'all'"
        :class="[
          'flex-1 py-2 font-medium transition-all cursor-pointer',
          activeTab === 'all'
            ? 'text-[#1890ff] border-b-2 border-[#1890ff] bg-white font-bold'
    ...
    <div class="flex-1 overflow-y-auto p-3 space-y-2 bg-white">
      <div v-if="filteredWidgets.length === 0" class="text-center py-6 text-gray-400 text-xs">
        未找到相关组态器件
      </div>
      <div
        v-else
        ...
        class="group flex gap-3 p-2.5 bg-[#fafafa] hover:bg-white border border-[#f0f0f0] hover:border-[#1890ff] hover:shadow-sm rounded cursor-pointer transition-all duration-200"
      >
        <div class="w-10 h-10 rounded bg-white border border-[#f0f0f0] flex items-center justify-center group-hover:scale-105 transition-all shadow-sm">
          <!-- Render icons -->
          <component
          ...
          />
          <div v-else-if="widget.icon === 'div-h'" class="w-7 h-2 bg-slate-600 rounded-full" />
          <div v-else-if="widget.icon === 'div-v'" class="w-2 h-7 bg-slate-600 rounded-full" />
          <div v-else-if="widget.icon === 'div-led'" class="w-4 h-4 rounded-full bg-emerald-500 ring-2 ring-emerald-300 animate-pulse" />
        </div>
        <div class="flex-1 min-w-0">
          <div class="flex justify-between items-start">
            <h4 class="text-xs font-semibold text-gray-800 group-hover:text-[#1890ff] transition-colors">
              {{ widget.name }}
            </h4>
          ...
          <p class="text-[10px] text-gray-400 mt-0.5 truncate leading-relaxed">
            {{ widget.description }}
          </p>
        </div>
      </div>
    ...
    <!-- Instructions Footer -->
    <div class="p-3 bg-[#fafafa] border-t border-[#f0f0f0] text-[10px] text-gray-400 text-center select-none leading-relaxed">
