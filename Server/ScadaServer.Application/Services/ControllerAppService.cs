...
{
    /// <summary>
    /// 控制器应用服务（阶段 2，控制器/PLC 资产台账）。
    /// 仅资产登记（CRUD/列表/下拉），不产生任何采集行为；删除引用校验在阶段 3 追加。
    /// </summary>
    public class ControllerAppService : IControllerAppService
    {
        ...
        /// <summary>按主键获取控制器，不存在时返回 null。</summary>
        ...
        /// <summary>删除控制器：记录不存在时静默忽略；阶段 3 后追加"有连接引用不可删"校验。</summary>
